...
class ChangeUserExperienceCompanyNameCommand:
    user_id: UUID
    experience_info_id: UUID
    company_name: str


class ChangeUserExperienceCompanyName:
    def __init__(self, repository: UserRepository, unit_of_work: UnitOfWork, event_bus: EventBus) -> None:
        self.repository = repository
        self.unit_of_work = unit_of_work
        self.event_bus = event_bus

    async def execute(self, command: ChangeUserExperienceCompanyNameCommand) -> None:
        ...
        user.change_experience_company_name(command.experience_info_id, command.company_name)
