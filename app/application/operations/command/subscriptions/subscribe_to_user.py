...
class RemoveSubscriber:
    ...
    async def execute(self, command: SubscrubeToUserCommand) -> None:
        ...
        if not receiver:
            raise UserNotFoundError(title=f"Receiver with id: {command.receiver_id} does not exist")

        await self.subscription_service.subscribe_to_user(subscriber=user, receiver=receiver)
