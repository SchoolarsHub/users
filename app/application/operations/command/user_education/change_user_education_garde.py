...
class ChangeUserEducationGradeCommand:
    user_id: UUID
    education_id: UUID
    education_grade: str


class ChangeUserEducationGrade:
    def __init__(self, repository: UserRepository, uow: UnitOfWork, event_bus: EventBus) -> None:
        self.repository = repository
        self.uow = uow
        self.event_bus = event_bus

    async def execute(self, command: ChangeUserEducationGradeCommand) -> None:
        ...
        user.change_education_grade(command.education_id, command.education_grade)
